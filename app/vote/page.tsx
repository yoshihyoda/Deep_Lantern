...
export default function VotePage() {
  ...
  useEffect(() => {
    const d = Number(new URLSearchParams(location.search).get('depth') ?? 2000);
    if (Number.isFinite(d) && d >= 0 && d <= 11000) setDepth(Math.round(d));
  }, []);
  return (
    <main className="vote-page">
      ...
      <div className="virtual-notice">
        Hypothetical / virtual exploration time. No real NOAA ROV time is
        allocated.
      </div>
      <Voting minDepth={depth} />
    </main>
  );
}
